...
impl Db {
    ...
    ///
    /// Reste le cas que la sonde ne voit pas : **tous les jetons denses, mais
    /// se croisant rarement** (deux mots courants qui ne cohabitent pas dans un
    /// même message). La descente par récence balaie alors tout l'historique.
    /// Ce n'est pas un défaut du choix : c'est précisément là que le plan par
    /// index est le plus cher, puisqu'il matérialise puis trie deux ensembles
    /// énormes pour n'en garder presque rien. Le balayage complet est mesuré à
    /// ~79 ms sur 100 000 messages, contre ~317 ms pour l'index — le pire cas
    /// reste donc du bon côté, et surtout il reste borné.
