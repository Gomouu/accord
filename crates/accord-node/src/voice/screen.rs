//! Fragmentation et réassemblage temps réel des trames de partage d'écran (v5).
//!
//! L'émetteur découpe chaque trame vidéo encodée (WebCodecs, côté UI) en
//! tranches ≤ [`CHUNK`], portées par des [`VoiceMsg::ScreenFrame`] — chacune un
...
/// Taille cible d'une tranche encodée. Reste sous `MAX_SCREEN_FRAGMENT` du
/// proto (1200) pour que le `ScreenFrame` scellé tienne dans un datagramme UDP.
const CHUNK: usize = 1000;

/// Borne anti-DoS d'une trame réassemblée (une keyframe de partage d'écran
/// raisonnable reste très en dessous).
...
/// Découpe une trame encodée en `ScreenFrame`s prêts à émettre. Une trame vide
/// produit un unique fragment vide (utile pour un signal de fin de trame).
pub(crate) fn fragment(
    ...
    let flags = if keyframe { SCREEN_FLAG_KEYFRAME } else { 0 };
    ...
    slices
        ...
        .enumerate()
        .map(|(idx, slice)| VoiceMsg::ScreenFrame {
            room,
            frame_id,
            frag_count,
            frag_idx: idx as u16,
...
mod tests {
    ...
    fn fragment_then_reassemble_roundtrips() {
        ...
        let msgs = fragment([7; 16], 1, true, &data);
    ...
    fn single_fragment_frame_completes_immediately() {
        let msgs = fragment([0; 16], 5, false, b"hello");
    ...
    fn lost_fragment_drops_the_frame() {
        let data: Vec<u8> = (0..2500u32).map(|i| i as u8).collect();
        let msgs = parts_of(&fragment([1; 16], 10, true, &data));
        let mut r = Reassembler::default();
    ...
    fn newer_frame_supersedes_incomplete_older_frame() {
        let a = parts_of(&fragment([1; 16], 10, true, &vec![0xAA; 2500]));
        let b = parts_of(&fragment([1; 16], 11, true, &vec![0xBB; 1500]));
        let mut r = Reassembler::default();
    ...
    fn stale_older_frame_is_ignored() {
        let newer = parts_of(&fragment([2; 16], 100, false, b"new"));
        let older = parts_of(&fragment([2; 16], 99, false, b"old"));
        let mut r = Reassembler::default();
    ...
    fn frame_id_wraparound_is_handled() {
        ...
        let a = parts_of(&fragment([3; 16], u32::MAX, false, b"a"));
        let b = parts_of(&fragment([3; 16], 0, false, b"b"));
        let mut r = Reassembler::default();
        assert!(r
