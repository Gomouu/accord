...
#[tokio::test]
async fn avatar_et_banniere_poses_apres_coup_arrivent_chez_l_ami_en_ligne() {
    ...
    // Alice pose avatar puis bannière APRÈS l'amitié (le cas du terrain).
    alice
        .node
    ...
    let avatar = alice
        .node
        .profile_set_avatar(Some(("image/png", image(1))))
        .unwrap()
        .expect("hash d'avatar");
    let banner = alice
        .node
        .profile_set_banner(Some(("image/png", image(2))))
        .unwrap()
        .expect("hash de bannière");

    // Bob doit finir avec les DEUX blobs en local (annonce → hash →
    // récupération automatique auprès d'Alice).
    assert!(
        attendre(|| matches!(bob.node.files_local_path(&avatar), Ok(Some(_)))).await,
        "octets de l'avatar jamais récupérés par l'ami"
    );
    assert!(
        attendre(|| matches!(bob.node.files_local_path(&banner), Ok(Some(_)))).await,
        "octets de la bannière jamais récupérés par l'ami"
    );
