...
use super::groups_audit::audit_entry_json;
use super::groups_members::members_page;
use super::helpers::{
...
pub(super) fn dispatch(node: &Node, method: &str, params: &Value) -> Result<Value, NodeError> {
    match method {
        ...
        // Lecture paginée de la seule liste non bornée de `groups.state`
        // (jalon 6, ROADMAP §18.3). `groups.state` continue de rendre la
        // liste entière : rien n'est retiré à un client tiers existant.
        "groups.members" => members_page(node, params),
        "groups.rename" => {
            let gid = param_id16(params, "group_id")?;
